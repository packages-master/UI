...
class TextViewUnSelected @JvmOverloads constructor(context: Context, var attrs: AttributeSet? = null, defStyleAttr: Int = 0, defStyleRes: Int = 0) : MaterialTextView(context, attrs, defStyleAttr, defStyleRes) {
    ...
    private fun initTextColor() {
        ThemeManager.getThemeManager().onLightThemeSelected {
            setTextColor(ResourcesCompat.getColor(resources, R.color.un_selected_text_color_light, null))
        }
        .onDarkThemeSelected {
            setTextColor(ResourcesCompat.getColor(resources, R.color.un_selected_text_color_dark, null))
        }
